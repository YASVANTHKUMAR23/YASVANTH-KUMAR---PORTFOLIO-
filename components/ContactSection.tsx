        ...
        <div className="w-full lg:w-1/2 text-left">
          ...
          >
            <h2 className="text-5xl md:text-8xl font-display font-bold text-white mb-8 tracking-tighter uppercase leading-[0.9]">
              {(data.heading || "Get In Touch").split(' ').map((word, i) => (
            ...
            <div className="space-y-6">
              <ContactInfoItem icon={<Mail size={20} />} label="Email Me" value={data.email} link={`mailto:${data.email}`} />
              <ContactInfoItem icon={<MessageSquare size={20} />} label="LinkedIn" value={data.linkedin} />
              <ContactInfoItem icon={<MapPin size={20} />} label="Location" value={data.location} />
            </div>
          </motion.div>
        </div>
          ...
          >
            <div className="space-y-6">
              <div className="relative">
                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] mb-3 block text-left">{data.labelName}</label>
                <div className="relative group/input">
                  <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within/input:text-[#00e1ab] transition-colors" />
                  <input
                    type="text"
              ...
              <div className="relative">
                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] mb-3 block text-left">{data.labelEmail}</label>
                <div className="relative group/input">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within/input:text-[#00e1ab] transition-colors" />
                  <input
                    type="email"
              ...
              <div className="relative">
                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em] mb-3 block text-left">{data.labelMessage}</label>
                <textarea
                  rows={4}
...
function ContactInfoItem({ icon, label, value, link }: { icon: any; label: string; value: string; link?: string }) {
  const content = (
